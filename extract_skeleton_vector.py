...
####################################################################################
# 데이터셋 구축을 위한 코드 (다수의 영상을 동시에 처리 가능함)
# 하는일 : skeleton vector를 추출하고, Handcraft feature를 계산해 저장함
####################################################################################
    ...
    for iv,v in enumerate(video_filenames):
      ...
      for i,f in enumerate(filenames):
          ...
          keypoint_coords *= output_scale
            ...
            # 웹에서 스켈레톤 벡터 추출 시각화를 위해 파일을 저장(이미지에 스켈레톤 벡터가 표시됨)
          if args.output_dir:
          ...
          if not args.notxt:
              ...
              if pose_scores[max_index] != 0. :
                  ...
                  out_data['skeleton_vector'] = tmp_data
                    ...
                    # Hand Craft Feature로의 변환이 완료되고, 해당 파일을 json 형식으로 저장
